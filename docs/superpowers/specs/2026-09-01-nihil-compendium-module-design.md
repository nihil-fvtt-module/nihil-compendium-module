...
- 첫 실제 팩은 `common-monsters` Actor 팩이다. 팩은 `공통` 팩 폴더에 속하며, GM과 보조 GM이 편집할 수 있고 플레이어에게는 표시하지 않는다.
- 팩 내부에는 `일반`, `엘리트`, `보스` Actor 폴더를 둔다.
- 각 폴더에는 CR 1부터 CR 30까지의 D&D5e NPC 액터를 둔다. 이름은 각각 `일반 CR N`, `엘리트 CR N`, `보스 CR N`으로 통일한다.
- 각 액터는 향후 스탯블록 작성의 출발점이며, 이번 단계에서는 CR과 최대·현재 HP만 입력한다. AC·공격·특성·이미지는 범위 밖이다.
- HP는 `1. 프롬프트/4. 룰북 관련 요청 프롬프트/1. 룰북 관련 요청 지침서.md`의 CR별 HP 기준표에서 해당 등급 범위의 산술 평균을 올림한 값으로 한다.
- 실제 팩 데이터베이스는 Foundry에서 생성·기록한다. 빈 `.db` 또는 LevelDB 파일을 수동으로 만들지 않는다.

## 추가 검증 기준

6. `common-monsters`가 Actor 팩으로 모듈 매니페스트에 등록되고, Compendium 탭의 `공통` 폴더에 표시된다.
7. 팩에는 `일반`, `엘리트`, `보스` 폴더와 각 30개, 총 90개의 액터가 존재한다.
8. 모든 액터의 이름·CR·현재 HP·최대 HP가 기준표의 중앙값과 일치한다.
